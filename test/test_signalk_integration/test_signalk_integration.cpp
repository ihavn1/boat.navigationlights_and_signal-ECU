...
BoatState test_stringToBoatState(const char* str) {
    ...
    if (strcmp(str, "towing") == 0) return BoatState::TOWING;
...
void test_boat_state_to_string_towing() {
    TEST_ASSERT_EQUAL_STRING("towing", test_boatStateToString(BoatState::TOWING));
}
...
void test_string_to_boat_state_towing() {
    TEST_ASSERT_EQUAL(BoatState::TOWING, test_stringToBoatState("towing"));
}
...
void test_boat_state_roundtrip_conversion() {
    ...
    TEST_ASSERT_EQUAL(BoatState::TOWING, 
        test_stringToBoatState(test_boatStateToString(BoatState::TOWING)));
}
...
int main(int argc, char **argv) {
    ...
    RUN_TEST(test_boat_state_to_string_towing);
    ...
    RUN_TEST(test_string_to_boat_state_towing);
